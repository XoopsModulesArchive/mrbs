...
if (!getAuthorised(4) and ($use_limit_time_booking))
{
	//xoops datelimit booking ahead
	$date_limit = time() + (86400 * ($time_booking));
	$time_limit = strftime("%x",$date_limit) ;
	$dateDiff = mktime(0,0,0,$month, $day, $year);
	if ($dateDiff > $date_limit) {
	?>
	<p>
		<a href="<?php echo htmlspecialchars($HTTP_REFERER); ?>">
		<h1><?php echo get_vocab("datelimit"). $time_limit. '<br />'; ?></h1>
...
//echo $create_by;
//echo XoopsUser::getUnameFromId(htmlspecialchars($row['create_by']));
//echo XoopsUser::getUnameFromId(htmlspecialchars($create_by));
...
<form class="form_general" id="main" action="edit_entry_handler.php" method="get">
  ...
  <legend><?php
		if (!getAuthorised(4) and ($use_limit_time_booking))
		{
		echo get_vocab("datelimit").$time_limit. get_vocab("datelimit2"); //xoops
		}
		?></legend>
