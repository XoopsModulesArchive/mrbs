...
include "dbsys.inc";
include "mrbs_auth.inc"; //xoops - blueteen
 ...
 <table id="entry">
   ...
   <tr>
       <!-- xoops - blueteen - convert UID -> username + link to profile -->
       	...
       	switch ($showbooker) {
           case "uname":
       	echo "<tr class=\"createdby\"><td>"  . get_vocab("createdby")  . ":</td>";
       	if (XoopsUser::getUnameFromId(htmlspecialchars($row['create_by'])) == $GLOBALS['xoopsConfig']['anonymous'])	{
       		echo "<td>".XoopsUser::getUnameFromId(htmlspecialchars($row['create_by']))."</td>";}
       		else {
       			 echo "<td><a href=\"../../userinfo.php?uid=".htmlspecialchars($row['create_by'])."\">".XoopsUser::getUnameFromId(htmlspecialchars($row['create_by']))."</a></td>";}
       	echo "</tr>\n";
       	break;
           case "name":
       	echo "<tr class=\"createdby\"><td>"  . get_vocab("createdby")  . ":</td>";
       	if (XoopsUser::getUnameFromId(htmlspecialchars($row['create_by'])) == $GLOBALS['xoopsConfig']['anonymous'])	{
       		echo "<td>".XoopsUser::getUnameFromId(htmlspecialchars($row['create_by']))."</td>";}
       			 ...
       			 echo "<td><a href=\"../../userinfo.php?uid=".htmlspecialchars($row['create_by'])."\">".XoopsUser::getUnameFromId(htmlspecialchars($row['create_by']),$usereal=1)."</a></td>";}
       	echo "</tr>\n";
       	break;
           case "both":
       	echo "<tr class=\"createdby\"><td>"  . get_vocab("createdby")  . ":</td>";
       	if (XoopsUser::getUnameFromId(htmlspecialchars($row['create_by'])) == $GLOBALS['xoopsConfig']['anonymous'])	{
       		echo "<td>".XoopsUser::getUnameFromId(htmlspecialchars($row['create_by']))."</td>";}
       			 ...
       			 echo "<td><a href=\"../../userinfo.php?uid=".htmlspecialchars($row['create_by'])."\">".XoopsUser::getUnameFromId(htmlspecialchars($row['create_by']))." (".XoopsUser::getUnameFromId(htmlspecialchars($row['create_by']),$usereal=1).")</a></td>";}
       	echo "</tr>\n";
       	break;
           case "none"://do not display booker
       	if (getAuthorised(2))//except for admin
       	{
       	echo "<tr class=\"createdby\"><td>"  . get_vocab("createdby")  . ":</td>";
       	if (XoopsUser::getUnameFromId(htmlspecialchars($row['create_by'])) == $GLOBALS['xoopsConfig']['anonymous'])	{
       		echo "<td>".XoopsUser::getUnameFromId(htmlspecialchars($row['create_by']))."</td>";}
...
<div id="view_entry_nav">
  <div>
    <?php
      if (getAuthorised(2)and (getWritable($create_by, getUserName()))) //xoops
      {
          if (! $series)
          {
